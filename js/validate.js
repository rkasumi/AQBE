...
var addValidate, errorWindow, validate, validateDvCount, validateDvDateTime, validateDvQuantity;
...
validate = function() {
  ...
  return $("#fieldDate").AnyTime_picker({
    format: "%Y/%m/%d %H:%i:%s",
    earliest: "1980/01/01 01:00:00",
    latest: new Date()
  });
...
addValidate = function(obj) {
  ...
  if (type === "DvQuantity") {
    return $(obj).unbind().change(function() {
      return validateDvQuantity(obj);
    });
  } else if (type === "DV_COUNT") {
    return $(obj).unbind().change(function() {
...
validateDvQuantity = function(obj) {
  ...
  if (!((min <= value && value <= max) || $(obj).val() === "")) {
    return errorWindow($(obj).next(), "Require => min: " + min + " max: " + max);
  } else {
    return errorWindow($(obj).next(), "");
  }
};
